...
const ZH: Record<string, string> = {
  ...
  phase2: '记忆整理中',
  processing: '记忆处理中',
  transfer_hint: '导出为一个 gzip 压缩的 bundle 文件;导入按内容哈希去重合并,冲突以本地为准,未压缩的 JSON 也能读。',
  transfer_exportLabel: '导出记忆',
  transfer_exportDesc: '保存为单个 bundle 文件,位置在浏览器里选',
  transfer_importLabel: '导入记忆',
  transfer_importDesc: '选择一个 bundle 文件合并进当前记忆库',
  transfer_export: '导出',
  transfer_import: '导入',
  transfer_requested: '已发送,结果见上方状态栏',
  transfer_tooBig: '文件超过 32 MB,请改用 memory_export 工具直接导出到磁盘',
  transfer_readFailed: '文件读取失败',
  viewer_title: '记忆浏览',
  viewer_refresh: '刷新',
  ...
  viewer_stat_indexSize: '注册表大小',
  viewer_topCited: '最常引用',
  viewer_noCitations: '还没有引用记录',
...
const EN: Record<string, string> = {
  ...
  transfer_hint: 'Export produces one gzip-compressed bundle file; import merges by content hash (conflicts keep the local copy) and also accepts uncompressed JSON.',
  transfer_exportLabel: 'Export memory',
  transfer_exportDesc: 'Save as a single bundle file — pick the location in the browser',
  transfer_importLabel: 'Import memory',
  transfer_importDesc: 'Pick a bundle file to merge into the current memory store',
  transfer_export: 'Export',
  transfer_import: 'Import',
  transfer_requested: 'Sent — result appears in the status bar above',
  transfer_tooBig: 'File exceeds 32 MB — use the memory_export tool to write it to disk instead',
  transfer_readFailed: 'Failed to read the file',
  viewer_title: 'Memory',
  ...
  viewer_stat_indexSize: 'Registry size',
  viewer_topCited: 'Most cited',
  viewer_noCitations: 'No citations yet',
  ...
  ctx.slots.inject('settings.section', () => ctx.slots.register(
    {
      ...
      inject: () => ({
        ...
        // useMemoryPage/useMemoryStatus/useMemoryView selector hooks.
        ...card.inject(),
        commands: commandsView,
        hooks: {
          memoryPage: card.pageStore(),
          memoryStatus: statusSource,
          memoryView: viewSource,
        },
      }),
    },
    MemorySettingsPage,
